...
object UserCtrlScala extends Controller {
  ...
  def getUserInfo(userId: Long) = Action.async(block = request => {
    ...
    val future = (for {
      ...
      zones <- getExpertZone(userProfile map (_.getZone.toSeq) getOrElse Seq())
    } yield {
      ...
      val expertInfo = new ObjectMapper().createObjectNode()
      if (userProfile nonEmpty) {
        val zoneStr: java.util.List[String] = zones.toList
        val travelNote = userProfile.get.getTravelNote

        node.put("travelNoteCnt", if (travelNote != null) {
          travelNote.size()
        } else 0)
        node.set(ExpertInfo.fnTags, new ObjectMapper().valueToTree(userProfile.get.getTags))

        // 达人信息
        expertInfo.put(ExpertInfo.fnProfile, userProfile.get.getProfile)
        expertInfo.set(ExpertInfo.fnZone, new ObjectMapper().valueToTree(zoneStr))
      } else {
        node.set("travelNoteCnt", null)
        node.set(ExpertInfo.fnTags, null)

        // 达人信息
        expertInfo.set(ExpertInfo.fnProfile, null)
        expertInfo.set(ExpertInfo.fnZone, null)
      }
  ...
  def getExpertZone(zoneIds: Seq[ObjectId]) = {
  ...
  def login() = Action.async(request => {
    val ret = for {
      ...
      if (password.nonEmpty && loginName.nonEmpty) {
        val name = try {
          PhoneParserFactory.newInstance().parse(loginName.get).getPhoneNumber
        } catch {
          case _: IllegalArgumentException => loginName.get.trim
        }
        val future = FinagleFactory.client.login(name, password.get, "app") map (user => {
  ...
  def searchExpert(zones: String) = Action.async(block = request => {
