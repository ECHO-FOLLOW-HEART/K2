...
object POICtrlScala extends Controller {
  def viewPOIInfo(poiDesc: String,
    @Key(tag = "poiId") spotId: String,
    @Key(tag = "cmtPage") commentPage: Int,
    @Key(tag = "cmtPageSize") commentPageSize: Int,
    @Key(tag = "rmdPage") rmdPage: Int,
    @Key(tag = "rmdPageSize") rmdPageSize: Int) = Action {
    request =>
      {
        //   获取图片宽度
