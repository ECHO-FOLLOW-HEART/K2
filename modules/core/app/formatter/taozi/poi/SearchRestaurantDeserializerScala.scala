 ...
 */
class SearchRestaurantDeserializerScala extends JsonDeserializer[Restaurant] {
  override def deserialize(p: JsonParser, ctxt: DeserializationContext): Restaurant = {
    ...
    restaurant.setId(new ObjectId(id))
    restaurant.setZhName(zhName)
    restaurant.setImages(seqAsJavaList(images))
