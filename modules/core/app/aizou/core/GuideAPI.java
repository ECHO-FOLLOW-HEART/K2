...
public class GuideAPI {
     ...
     * @return
     */
    public static Guide fillGuideInfo(Guide guide) throws TravelPiException {
        if (guide == null)
            return new Guide();
        AbstractPOI poi;
        String type;
            ...
            for (ItinerItem temp : itinerary) {
                ...
                switch (type) {
                    ...
                    case "shopping":
                        ...
                        break;
                    default:
                        throw new TravelPiException(ErrorCode.INVALID_ARGUMENT, String.format("Invalid POI type: %s.", type));
                }
                poi = PoiAPI.getPOIInfo(temp.poi.id, poiType, true);
                if (poi == null) {
                    LogUtils.info(GuideAPI.class, String.format("POI is not exist.Id: %s, Type: %s.", temp.poi.id.toString(), type));
                    continue;
                } else {
                    temp.poi = poi;
