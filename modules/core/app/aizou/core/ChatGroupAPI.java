...
public class ChatGroupAPI {
     ...
     * 发送rest请求,获取response
     */
    public static String setUrlConnection(String href, JsonNode node, String httpmethod) throws IOException, TravelPiException {
    ...
    public static String createGroupApi(Integer ownerId, String groupName, String desc, boolean isGroupPublic, Integer maxusers) throws TravelPiException {
        ...
        UserInfo owner = UserAPI.getUserByUserId(ownerId, Arrays.asList(UserInfo.fnNickName, UserInfo.fnAvatar,
                UserInfo.fnSignature, UserInfo.fnGender));
    ...
    public static String deleteGroupApi(String groupId, Integer id) throws TravelPiException {
        ...
        if (chatGroupInfo.owner.userId == id) {       //判断id,删除操作只能由群主发起
            ...
            String data = "";
            try {
                String body = setUrlConnection(href, Json.newObject(), "DELETE");
                JsonNode dataNode = Json.parse(body);
    ...
    public static void putUserIntoGroupApi(String groupId, List<Integer> userList) throws TravelPiException {
        ...
        for (Integer id : userList) {
            UserInfo userInfo = UserAPI.getUserByUserId(id, Arrays.asList(UserInfo.fnNickName,
    ...
    public static void deleteMemberFromGroupApi(String groupId, List<Integer> userList) throws TravelPiException {
        ...
        if (userList != null) {
            for (Integer id : userList) {
                ...
                try {
                    setUrlConnection(href, Json.newObject(), "DELETE");
    ...
    public static void modifyChatGroupDetailApi(String groupId, String id, boolean isGroupPublic, String groupName, String desc) throws TravelPiException {
        /**
         * 本地库的更新
         */
        ChatGroupInfo chatGroupInfo = getUserGroupInfo(groupId, Arrays.asList(ChatGroupInfo.GROUPNAME, ChatGroupInfo.DESC,
                ChatGroupInfo.ISGROUPPUBLIC));
        if (chatGroupInfo == null) {
            throw new TravelPiException(ErrorCode.INVALID_ARGUMENT, "INVALID_ARGUMENT");
        }
        if (chatGroupInfo.owner.id.equals(id + "")) {
            if (desc != null)
                chatGroupInfo.desc = desc;
            if (groupName != null)
                chatGroupInfo.groupName = groupName;
