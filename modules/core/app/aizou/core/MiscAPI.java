...
public class MiscAPI {
     ...
     * @throws TravelPiException
     */
    public static List<Comment> displayCommentApi(String poiId,int page,int pageSize) throws TravelPiException {
        Datastore ds=MorphiaFactory.getInstance().getDatastore(MorphiaFactory.DBType.MISC);
        Query<Comment> query=ds.createQuery(Comment.class).field("poiId").equal(poiId);
        return query.order("-commentTime").offset(page * pageSize).limit(pageSize).asList();
    }
