...
object TravelNoteAPI {
  ...
  def getTravelNote(itemIds: Seq[ObjectId])(implicit ds: Datastore, futurePool: FuturePool): Future[Option[Seq[TravelNote]]] = futurePool {
    if (itemIds.size == 0)
      ...
      query.retrievedFields(true, Seq(TravelNote.fnId, TravelNote.fnTitle, TravelNote.fnImages, TravelNote.fnSummary, TravelNote.fnTravelTime, TravelNote.fnAuthorAvatar): _*)
      Some(query.asList())
    }
  }
